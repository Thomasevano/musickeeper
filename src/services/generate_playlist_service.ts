...
async function generatePlaylistFile(accessToken:string, playlistUrl: string, playlistName:string) {
  let url = playlistUrl
  let result = await fetchSongsFromPlaylist(accessToken, url)
  let nextUrl = result.next
  const totalItems = result.total
  const tracks: Array<string> = []
  while (tracks.length < totalItems) {
    if (url != playlistUrl) {
      result = await fetchSongsFromPlaylist(accessToken, url)
      nextUrl = result.next
    }
