...
<div class={cn("space-y-3", className)} {...$$restProps}>
	...
	<Button on:click={async () => {await generatePlaylistFile(spotifyAccessToken, album.tracks.href, album.name)}}>
		<Download class="mr-2 h-4 w-4" />
		To Text
	</Button>
	<div class="space-y-1 text-sm">
		<h3 class="font-medium leading-none">{album.name}</h3>
		<p class="text-xs text-muted-foreground">{ album.type === "playlist" ? album.description : album.artists[0]}</p>
	</div>
</div>
