<script lang="ts">
  ...
  import AddItemToListenLaterButton from '~/components/AddItemToListenLaterButton.svelte'
        ...
        <Command.List>
          ...
          <Command.Group heading="Tracks">
            {#each matchingItems.tracks.items as track (track.id)}
              <Command.Item class="flex items-center gap-2">
                <img
                ...
                <AddItemToListenLaterButton {listenLaterItems} {track} />
              </Command.Item>
            {/each}
          </Command.Group>
        </Command.List>
    ...
    <div>
      {#if listenLaterItems.length > 0}
        <table>
          <thead>
            <tr>
              <th class="px-4 py-2">Has Been Listened</th>
