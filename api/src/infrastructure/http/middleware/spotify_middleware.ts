...
export default class SpotifyMiddleware {
  async handle(ctx: HttpContext, next: NextFn) {
    const spotifyAccessTokenExpiresAt = ctx.request.plainCookie('spotify_access_token_expires-at')
    const spotifyRefreshToken = ctx.request.encryptedCookie('spotify_refresh_token')
