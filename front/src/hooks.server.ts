...
export const handle: Handle = async ({ event, resolve }) => {
  ...
  const difference = Math.floor((Number.parseInt(spotifyAccessTokenExpiresAt) - now) / 60);

  console.log({ spotifyAccessToken, spotifyAccessTokenExpiresAt, now, difference })

  if (spotifyAccessToken && difference < 3) {
    try {
